...
function computeProviderStats(d: OddsMovementData) {
  ...
  if (vals.length === 0) return { first: undefined, last: undefined, changePct: 0 };
  const first = vals[0];
...
function ModalInner({
    ...
    import("lightweight-charts").then(({ createChart, AreaSeries, LineSeries, ColorType, CrosshairMode, LineStyle }) => {
      ...
      chart = createChart(container, {
        ...
        grid: {
          ...
          horzLines: { color: "rgba(148, 163, 184, 0.06)", style: LineStyle.Dashed },
        },
        crosshair: {
  ...
  const featureGroups = hasFeatures ? (() => {
    const grouped = new Map<string, { meta: typeof FEATURE_CATALOG[number]; value: number }[]>();
  ...
  return (
    <div className="flex relative w-full bg-[#0c0e14] selection:bg-white/10">
      ...
      <div className={cn("flex-1 flex flex-col min-w-0", hasFeatures && "mr-[300px]")}>
        {/* ── Header with gradient accent ── */}
        <div className={cn("relative shrink-0 overflow-hidden", `bg-gradient-to-r ${accentGradient}`)}>
        {/* Decorative top bar */}
        <div className={cn(
          "absolute top-0 inset-x-0 h-[2px]",
          isUp ? "bg-gradient-to-r from-emerald-500/0 via-emerald-400/80 to-emerald-500/0"
            : isDown ? "bg-gradient-to-r from-rose-500/0 via-rose-400/80 to-rose-500/0"
                ...
                <TooltipTrigger asChild>
                  ...
                  >
                    ...
                    <span className={cn(
                      ...
                      hidden ? "text-white/30 line-through" : "text-white/80",
                    )}>
                      {getProviderShortName(providerId)}
                    </span>
                  </button>
                </TooltipTrigger>
                <TooltipContent side="bottom" className="text-xs bg-slate-900 border-white/10">
                  {hidden ? "Show" : "Hide"} {getProviderShortName(providerId)}
                </TooltipContent>
              </Tooltip>
            );
          })}
          ...
          <div className="shrink-0 border-t border-white/[0.05] bg-black/20 p-4">
             ...
             <div className="flex flex-col gap-1 mt-1">
              {Object.entries(dataMap).map(([providerId, providerData]) => {
                if (!providerData || providerData.sparkline.length < 2) return null;
                const labelColor = getProviderChartHex(providerId);
                const { last: pLast, changePct: pChangePct } = computeProviderStats(providerData);
                const pDirColor = pChangePct > 0.01 ? "text-emerald-400" : pChangePct < -0.01 ? "text-rose-400" : "text-white/40";
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      <div className="relative flex items-center justify-center size-5 rounded-full bg-black/40 ring-1 ring-white/10 shadow-inner">
                        <span
                          ...
                          style={{ backgroundColor: labelColor, boxShadow: `0 0 8px ${labelColor}` }}
                        />
                      </div>
                    ...
                    <span className={cn("text-right font-mono text-[11px] font-semibold flex items-center justify-end gap-1", pDirColor)}>
                      <span className="opacity-80 text-xs">{pDirArrow}</span>
                      <span>{pChangePct > 0 ? "+" : ""}{pChangePct.toFixed(2)}%</span>
                    </span>
              ...
              {Array.from(featureGroups.entries()).map(([cat, items]) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 mb-2.5">
                    <span className={cn("size-1.5 rounded-full shrink-0 ring-4 ring-black/20", CATEGORY_COLORS[cat as keyof typeof CATEGORY_COLORS])} />
                    <span className={cn(
                      "text-[10px] font-bold uppercase tracking-widest",
                      CATEGORY_TEXT_COLORS[cat as keyof typeof CATEGORY_TEXT_COLORS],
                      "opacity-80",
                    )}>{cat}</span>
                  </div>
                  ...
                  <div className="flex flex-col gap-1">
                    {items.map(({ meta, value }) => (
                      <Tooltip key={meta.name}>
                        ...
                        <TooltipContent side="left" className="max-w-[240px] p-2.5 text-sm leading-relaxed bg-slate-900 border-white/10 shadow-xl">
                          <div className="font-semibold text-white/90 mb-0.5 text-xs">{meta.label}</div>
                          <div className="text-white/60 text-[11px] mb-2 leading-tight">{meta.desc}</div>
                          <div className="inline-block px-1.5 py-0.5 rounded bg-black/50 text-[10px] text-white/40 font-mono ring-1 ring-white/5">{meta.name} = {value}</div>
                        </TooltipContent>
                      </Tooltip>
                    ))}
                  </div>
