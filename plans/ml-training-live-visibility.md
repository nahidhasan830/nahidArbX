...
**Evidence**: 5 failed models in the DB, all with _"Training timed out — Cloud Run Job may have exited without updating the database."_ Current training (`cloud-training-1779040096444`) has been `training` for ~32 min with 0 samples. This message means Python exited without ever writing a terminal status — not a GCP timeout.
...
| `progressMessage`          | `text`      | Human-readable: _"Running HPO trial 23/50"_, _"Running CPCV fold 3/10"_, etc.                                         |
...
const age =
  now.getTime() -
  new Date(model.lastHeartbeatAt ?? model.trainingStartedAt).getTime();
```
