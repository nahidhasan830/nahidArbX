 ...
 * main-site (not from our `placed_bets` table) so it reflects the
...
import { placedBets, valueBets } from "@/lib/db/schema";
 ...
 * a row in our `placed_bets` table (matched via providerTicketId ↔
...
export async function GET() {
  ...
  // ── Enrichment: join against placed_bets for richer display fields.
  //
  // For any bet we also placed through our system, pull the event name,
  // selection label, market type, and home/away team (via value_bets).
  // Non-matches simply skip — the UI falls back to main-site fields.
  //
  // Match key: `placed_bets.providerTicketId` is set by the reconciler
  ...
  try {
    ...
    // we stored as `placed_bets.provider_ticket_id`. We probe both the
    ...
    if (ticketIds.size > 0) {
      const placedRows = await db
        .select()
        .from(placedBets)
        .where(inArray(placedBets.providerTicketId, [...ticketIds]));
      ...
      // Second hop: value_bets for home/away team names.
      const valueBetIds = Array.from(
        new Set(
          placedRows.map((p) => p.valueBetId).filter((v): v is string => !!v),
        ),
      );
      const vbByIdMap = new Map<
        string,
        { homeTeam: string; awayTeam: string; competition: string | null }
      >();
      if (valueBetIds.length > 0) {
        const vbRows = await db
          .select({
            id: valueBets.id,
            homeTeam: valueBets.homeTeam,
            awayTeam: valueBets.awayTeam,
            competition: valueBets.competition,
          })
          .from(valueBets)
          .where(inArray(valueBets.id, valueBetIds));
      ...
      for (const bet of all) {
        ...
        const placed =
          ...
          (bet.transactionId !== undefined && bet.transactionId !== null
            ...
            : undefined);
        if (!placed) continue;
        const vb = placed.valueBetId
          ? vbByIdMap.get(placed.valueBetId)
          : undefined;
        bet.enrichment = {
          placedBetId: placed.id,
          valueBetId: placed.valueBetId ?? undefined,
          eventName: placed.eventName,
          marketType: placed.marketType,
          atomLabel: placed.atomLabel,
          competition: placed.competition ?? vb?.competition ?? undefined,
