...
import { derive, settlementPnl } from "@/lib/bets-history/derive";
import { summarizeClv, winZScore, zToPValue } from "@/lib/bets-history/metrics";
import { normalizeOutcome } from "@/lib/bets-history/types";
  ...
  try {
    ...
    if (trainSize > 0 && trainSize < totalN) {
      const cutoff = await db
        .select({ firstSeenAt: bets.firstSeenAt })
        .from(bets)
        .orderBy(asc(bets.firstSeenAt))
        .limit(1)
        .offset(trainSize - 1);
    ...
    if (filters.tickMin != null)
      clauses.push(gte(bets.tickCount, filters.tickMin));
    if (filters.oddsMin != null)
      clauses.push(gte(bets.softOdds, filters.oddsMin));
    if (filters.oddsMax != null)
      clauses.push(lte(bets.softOdds, filters.oddsMax));
    if (filters.timeScope) clauses.push(eq(bets.timeScope, filters.timeScope));
    ...
    const avgEvPct =
      rows.reduce((s, r) => s + derive(r).evPct, 0) / rows.length;
