...
import { analyzeBacktest } from "@/lib/ai/analyze-backtest";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const rows =
      "ids" in parsed.data
        ? await getValueBetsByIds(parsed.data.ids)
        : (
            await listValueBets({
              ...parsed.data.filters,
    ...
    const analysis = await analyzeBacktest(rows, { model: parsed.data.model });
