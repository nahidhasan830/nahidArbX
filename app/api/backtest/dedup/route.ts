/**
 * One-off dedup endpoint — removes duplicate value_bets rows created by the
 * pre-fix matcher that produced unstable composite eventIds.
 *
 * POST /api/backtest/dedup?dryRun=true   → preview counts only
 * POST /api/backtest/dedup               → delete losing rows in each dup group
 *
...
export async function POST(request: NextRequest) {
  ...
  try {
            ...
            SELECT id,
                   ROW_NUMBER() OVER (
                     PARTITION BY home_team, away_team, event_start_time,
                                  family_id, atom_id
                     ORDER BY soft_odds_max DESC, first_seen_at ASC, id ASC
                   ) AS rn
            FROM value_bets
          ),
          deleted AS (
    ...
    const remaining = await db.execute<{ total: number }>(
      sql`SELECT COUNT(*)::int AS total FROM value_bets`,
    );
