...
import {
  ...
  listAiLogs,
  type AiLogFilters,
} from "@/lib/db/repositories/ai-logs";
...
const querySchema = z.object({
  ...
  endpoints: z.string().optional(), // comma-separated
...
export async function GET(req: NextRequest) {
  try {
    ...
    const filters: AiLogFilters = {
      ...
      endpoints: q.endpoints?.split(",").filter(Boolean),
      search: q.search,
      limit: q.limit,
      offset: q.offset,
    };
    ...
    if (q.aggregate === "true") {
      const stats = await aggregateAiLogs(filters);
      return NextResponse.json(stats);
    }

    const result = await listAiLogs(filters);
