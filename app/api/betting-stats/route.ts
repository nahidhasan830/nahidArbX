 ...
 *
 * Derives dashboard KPIs + breakdowns from the real `placed_bets` table.
...
function deriveStats(rows: PlacedBetRow[]) {
  ...
  return {
    totals: {
      ...
      expectedProfit: 0, // we don't track per-row EV on placed_bets yet
      luckDelta: 0,
...
function groupBy(
  rows: PlacedBetRow[],
  keyFn: (r: PlacedBetRow) => string,
  labelFn: (r: PlacedBetRow) => string,
): BreakdownRow[] {
  const map = new Map<string, { label: string; rows: PlacedBetRow[] }>();
  for (const r of rows) {
    const k = keyFn(r);
...
function buildPnlSeries(rows: PlacedBetRow[]): PnlPoint[] {
...
function computeMaxDrawdown(settled: PlacedBetRow[]): number {
...
function buildHeatmap(rows: PlacedBetRow[]) {
  const map = new Map<
    string,
...
function buildTopBets(settled: PlacedBetRow[], kind: "wins" | "losses") {
  return [...settled]
    .filter((r) => r.pnl !== null)
    .sort((a, b) =>
      kind === "wins"
...
function buildStreaks(settled: PlacedBetRow[]) {
  const chrono = [...settled].sort((a, b) =>
