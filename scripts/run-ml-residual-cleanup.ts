...
async function main(): Promise<void> {
  ...
  const result = await db.transaction(async (tx) => {
    // ── BEFORE ──
    const before: Counts = {
      beforeMlModels: await scalar(
        tx,
        sql`SELECT count(*)::int AS n FROM ml_models`,
      ),
      beforeMlModelsFv3: await scalar(
        tx,
        sql`SELECT count(*)::int AS n FROM ml_models WHERE feature_version <> 1`,
      ),
      beforeTrainingExamples: await scalar(
        tx,
        sql`SELECT count(*)::int AS n FROM ml_training_examples`,
      ),
      beforeTrainingExamplesFv3: await scalar(
        tx,
        sql`SELECT count(*)::int AS n FROM ml_training_examples WHERE feature_version <> 1`,
      ),
      beforeBetsFv3WithFeatures: await scalar(
        tx,
        sql`SELECT count(*)::int AS n FROM bets WHERE ml_features IS NOT NULL AND ml_feature_version <> 1`,
      ),
      beforeBetsFv1WithFeatures: await scalar(
        tx,
        sql`SELECT count(*)::int AS n FROM bets WHERE ml_features IS NOT NULL AND ml_feature_version = 1`,
      ),
    };
    ...
    const deletedModels = await scalar(
      tx,
      sql`
    ...
    const deletedExamples = await scalar(
      tx,
      sql`
    ...
    const updatedBets = await scalar(
      tx,
      sql`
      WITH u AS (
        UPDATE bets
    ...
    // ── AFTER ──
    const afterMlModels = await scalar(
      tx,
      sql`SELECT count(*)::int AS n FROM ml_models`,
    );
    const afterTrainingExamples = await scalar(
      tx,
      sql`SELECT count(*)::int AS n FROM ml_training_examples`,
    );
    const afterTrainingExamplesFv1 = await scalar(
      tx,
      sql`SELECT count(*)::int AS n FROM ml_training_examples WHERE feature_version = 1`,
    );
    const afterBetsFv1WithFeatures = await scalar(
      tx,
      sql`SELECT count(*)::int AS n FROM bets WHERE ml_features IS NOT NULL AND ml_feature_version = 1`,
    );
    const afterBetsAnyWithFeatures = await scalar(
      tx,
      sql`SELECT count(*)::int AS n FROM bets WHERE ml_features IS NOT NULL`,
    );
    ...
    const expectedAfterMlModels =
      before.beforeMlModels - before.beforeMlModelsFv3;
    if (afterMlModels !== expectedAfterMlModels) {
      errors.push(
        `ml_models invariant: after=${afterMlModels}, expected=${expectedAfterMlModels}`,
      );
    }
    ...
    if (afterTrainingExamples !== afterTrainingExamplesFv1) {
      errors.push(
        `ml_training_examples still has non-fv1 rows: total=${afterTrainingExamples}, fv1=${afterTrainingExamplesFv1}`,
      );
    }
    const expectedAfterExamples =
      before.beforeTrainingExamples - before.beforeTrainingExamplesFv3;
    if (afterTrainingExamples !== expectedAfterExamples) {
      errors.push(
        `ml_training_examples invariant: after=${afterTrainingExamples}, expected=${expectedAfterExamples}`,
      );
    }
    ...
    if (afterBetsFv1WithFeatures !== before.beforeBetsFv1WithFeatures) {
      errors.push(
        `v=1 corpus changed: before=${before.beforeBetsFv1WithFeatures}, after=${afterBetsFv1WithFeatures}`,
      );
    }
    ...
    if (afterBetsAnyWithFeatures !== afterBetsFv1WithFeatures) {
      errors.push(
        `bets still carry non-fv1 features: total=${afterBetsAnyWithFeatures}, fv1=${afterBetsFv1WithFeatures}`,
      );
    }
    ...
    if (deletedModels !== before.beforeMlModelsFv3) {
      errors.push(
        `deleted ml_models mismatch: deleted=${deletedModels}, expected=${before.beforeMlModelsFv3}`,
      );
    }
    if (deletedExamples !== before.beforeTrainingExamplesFv3) {
      errors.push(
        `deleted ml_training_examples mismatch: deleted=${deletedExamples}, expected=${before.beforeTrainingExamplesFv3}`,
      );
    }
    if (updatedBets !== before.beforeBetsFv3WithFeatures) {
      errors.push(
        `cleared bets mismatch: cleared=${updatedBets}, expected=${before.beforeBetsFv3WithFeatures}`,
      );
    }
