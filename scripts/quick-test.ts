...
async function main() {
  ...
  console.log(`Found ${eligible.length} eligible bets out of ${rows.length} settled`);
  if (eligible.length > 0) {
    const sample = eligible.slice(0, 5);
    for (const bet of sample) {
      const score = scores.get(bet.eventId)!;
      console.log(`${bet.homeTeam} vs ${bet.awayTeam} | ${bet.marketType} | Actual: ${score.ftHome}-${score.ftAway} | Outcome: ${bet.outcome}`);
    }
  }
}
