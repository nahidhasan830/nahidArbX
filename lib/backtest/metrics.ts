...
import { hasPnl, isSettledOutcome, type ValueBetRow } from "./types";
...
export const clvPct = (row: ValueBetRow): number | null => {
  ...
  return row.softOddsFirst / row.closingSharpOdds - 1;
};
...
export const sortino = (rows: ValueBetRow[]): number | null => {
  ...
  const decided = rows.filter((r) => hasPnl(r.outcome));
...
export const valueForDimension = (
  ...
  switch (dim) {
    ...
    case "evBucket":
      return evBucket(derive(row).evPctFirst);
    case "oddsBucket":
      return oddsBucket(row.softOddsFirst);
    case "tickBucket":
