...
export type BetsHistoryAnalysis = {
  summary: string;
  patterns: string[];
  concerns: string[];
...
const compactify = (row: ValueBetRow): CompactBet => ({
  ...
  soft_odds_first: row.softOdds,
  soft_odds_max: row.softOdds,
...
export const analyzeBetsHistory = async (
  rows: ValueBetRow[],
  options?: { model?: ModelTier | string },
): Promise<BetsHistoryAnalysis> => {
  ...
  let parsed: Omit<BetsHistoryAnalysis, "model">;
  try {
    parsed = JSON.parse(text);
  } catch (err) {
    logger.error("AnalyzeBetsHistory", `Bad JSON: ${text.slice(0, 200)}`);
