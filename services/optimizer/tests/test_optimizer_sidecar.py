"""Python optimizer sidecar hardening tests.
...
class TestCPCVEventPurging:
    """CPCV should purge ALL train rows sharing event_ids with test."""
...
class TestPBODemotedToWarning:
    """PBO should be a warning, not a hard rejection gate."""
...
class TestColdStartAlignment:
    """TS and Python cold-start thresholds should be aligned at 200."""
...
class TestSampleWeightParity:
    """Verify sample weight consistency between loaders."""

    def test_derive_sample_weights_half_outcomes(self):
        """Half outcomes should get reduced weight (0.5 base)."""
        rows = [
            {"outcome": "won", "soft_odds": 2.0, "soft_commission_pct": 0.0},
            {"outcome": "half_won", "soft_odds": 2.0, "soft_commission_pct": 0.0},
            {"outcome": "lost", "soft_odds": 2.0, "soft_commission_pct": 0.0},
            {"outcome": "half_lost", "soft_odds": 2.0, "soft_commission_pct": 0.0},
        ]
        weights = _derive_sample_weights(rows)
        assert weights[0] == 1.0   # won → |unit_return| = 1.0
        assert weights[1] == 0.25  # half_won → 0.5 return × 0.5 half-outcome weight
        assert weights[2] == 1.0   # lost → |unit_return| = 1.0
        assert weights[3] == 0.25  # half_lost → 0.5 loss × 0.5 half-outcome weight

    def test_derive_sample_weights_scales_with_unit_return(self):
        """Higher unit return magnitude should get higher sample weight."""
        rows = [
            {"outcome": "won", "soft_odds": 2.0, "soft_commission_pct": 0.0},
            {"outcome": "won", "soft_odds": 4.0, "soft_commission_pct": 0.0},
        ]
        weights = _derive_sample_weights(rows)
...
class TestUnitReturnConsistency:
    """Verify unit return computation is consistent."""
...
class TestLoaderCanonicalExamples:
    """One canonical training example per source bet."""
...
class TestLoaderFallbackClassification:
    """Distinguish empty corpus, missing table, and broken probe."""

    def test_missing_table_falls_back_cleanly(self):
        session = MagicMock()
        session.execute.side_effect = ProgrammingError(
            statement="SELECT count(*) FROM ml_training_examples",
            params={},
            orig=Exception('relation "ml_training_examples" does not exist'),
        )

        result = load_from_training_examples(session)

        assert result is None
        session.rollback.assert_called_once()

    def test_loader_probe_bug_raises_instead_of_silent_fallback(self):
        session = MagicMock()
        session.execute.side_effect = ProgrammingError(
            statement="SELECT count(*) FROM ml_training_examples WHERE feature_version = :version",
            params={},
            orig=Exception("bind parameter 'version' not supplied"),
        )

        with pytest.raises(RuntimeError, match="probe failed before fallback"):
            load_from_training_examples(session)

        session.rollback.assert_called_once()


class TestLoaderChronologicalMerge:
    """Merged data should be sorted chronologically."""
